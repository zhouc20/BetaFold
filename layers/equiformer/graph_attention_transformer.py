...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        drop_path_rate=0.0,
        mean=None, std=None, scale=None, atomref=None, max_num_neighbors=256):
        super().__init__()
    ...
    def forward(self, f_in, pos, batch, atom_embedding, **kwargs) -> torch.Tensor:
        """前向传播, f_in和kwargs似乎没用到

        Args:
            f_in (FloatTensor): (N, 5)
            pos (FloatTensor): (N, 3)
            batch (IntTensor): (N,)
        ...
        """
        edge_src, edge_dst = radius_graph(pos, r=self.max_radius, batch=batch, max_num_neighbors=self.max_num_neighbors)
